# ⚡ Kelvin Puch
> **Fullstack Developer & AI Foundations Certified** |

Combinando la robustez del Backend con el poder de la Inteligencia Artificial. Me especializo en arquitecturas modernas, desde el entrenamiento teórico de modelos hasta el despliegue de APIs escalables.

---

### Especialización en IA & Data Science
Certificado por **Oracle Cloud Infrastructure (OCI)** y **Cognitive Class (IBM)**:
* **Generative AI & LLMs:** Conocimiento profundo en Large Language Models, Fine-tuning y arquitecturas de IA moderna.
* **Data Science Foundations:** Extracción de patrones y optimización de toma de decisiones mediante análisis científico de datos.
* **Machine & Deep Learning:** Fundamentos teóricos aplicados al desarrollo de soluciones inteligentes.

---

### Mi Stack

| Área | Tecnologías |
| :--- | :--- |
| **Backend** | Python (**Django**), Java (**Spring Boot**), Hibernate/JPA |
| **Frontend** | **React.js**, JavaScript (ES6+), HTML5/CSS3, SASS, Tailwind |
| **AI/Data** | Jupyter, Oracle Data Science |
| **DevOps** | **Docker**, PostgreSQL/MySQL, Bash Scripting, Git/GitHub |


---

### Certificaciones Destacadas
* **Oracle:** AI Foundations Associate (ML, DL, LLMs).
* **CISCO:** Python Essentials.
* **CognitiveClass.ai:** SQL and Relational Databases.
* **EDteam:** Java desde Cero.

